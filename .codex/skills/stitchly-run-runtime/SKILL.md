...
5. Validate Rust changes with the narrow affected package check or test when practical, usually `scripts/dev_ui_agent.sh check core` or `scripts/dev_ui_agent.sh check server`; load `.codex/skills/stitchly-rust-quality/references/compile-routing.md` when choosing between check, build, and restart.
