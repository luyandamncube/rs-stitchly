...
- Backend storage/API changes: run `cargo test --workspace` or affected package tests.
- Frontend workspace/catalog changes: run `corepack pnpm --dir apps/web test --run`.
- Cross-stack catalog behavior may need the dev UI script: `npm run dev:ui:no-open`.
